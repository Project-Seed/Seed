Shader "Custom/Grow2"
{
	Properties
	{
		...
		_Grow2("Grow2",Range(-0.5,0.5)) = -0.5
		...
		SubShader
		{
			...
			void vert(inout appdata_full v) {
				float m = tex2Dlod(_MaskTex, float4(v.texcoord.x + _Grow2, v.texcoord.y , 0, 0)).r;
				float disp = tex2Dlod(_Displacement, float4(v.texcoord.x * _TileOff.x + _TileOff.z + _Grow2, v.texcoord.y * _TileOff.y + _TileOff.w , 0, 0)).r * _Dis; //
			...
			void surf(Input IN, inout SurfaceOutputStandard o)
			{
				...
				//fixed4 c = tex2D(_MainTex, float2(IN.uv_MaskTex.x + _Grow2, IN.uv_MaskTex.y));
				float m = tex2D(_MaskTex, float2(IN.uv_MaskTex.x + _Grow2,IN.uv_MaskTex.y)).r;
				o.Emission = c.rgb;
				o.Alpha = m;
			}
			ENDCG
